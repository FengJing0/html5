        ...
        <tbody>
          <tr v-for="(row, index) in items">
            ...
            <td v-for="col in headers"
                :key="col.value"
                :class="col.align==='right'?'text-xs-right':'text-xs-left'">
              <span v-if="!col.type">{{row[col.value]}}</span>
              <input v-else-if="col.type === 'default'"
              ...
              <div v-else-if="col.type==='select'"
                ...
                <select v-model="row[col.value]">
                  <option v-for="option in col.options"
                          :value="option.value">
                    {{option.text}}
                  </option>
                </select>
            ...
            <td v-if="actionsVisible">
              <v-btn v-if="editBtnVisible"
                     primary
                     small
                     @click.native="$emit('edit', {row, index})">Edit</v-btn>
              <v-btn v-if="removeBtnVisible"
                     error
                     small
                     @click.native="$emit('remove', {row, index})">Remove</v-btn>
            </td>
          </tr>
        </tbody>
      </slot>
