  ...
  <v-layout row
    ...
    <v-flex class=""
      ...
      <div style="width:250px;"
           class="mr-3">
        <v-select v-bind:items="programs"
                  item-text="name"
                  item-value="subr"
                  v-model="subr"
      ...
      <div class="flex flex-align-center"
        ...
        <v-dialog v-model="searchDialog"
          ...
          <v-card>
                ...
                <v-flex>
                  <form @submit.prevent="search"
                    ...
                    <v-layout row
                      ...
                      <v-flex xs6>
                        <v-select v-bind:items="['Subroutine', 'Program','Main','Manual', 'Auto']"
                                  v-model="searchMode"
                                  label="Search mode"
                                  bottom></v-select>
                      </v-flex>
                      <v-flex xs4>
                        <v-checkbox label="Match upper lower case"
                                    v-model="searchMULC"></v-checkbox>
                      </v-flex>
                      <v-flex xs8>
                        <v-checkbox label="Match whole word"
                                    v-model="searchMWW"></v-checkbox>
                      </v-flex>
                      <v-flex xs4>
                        <v-btn type="submit"
                               block
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn warning
                     @click.native="searchDialog = false">Close</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="newProgram.visible"
                  persistent>
          ...
          <v-card>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn warning
                     @click.native="newProgram.visible = false">Close</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>

        <v-btn v-if="program"
               @click="newItem">New Row</v-btn>
      </div>
    </v-flex>
        ...
        <Datatable :headers="headers"
          ...
          <tbody slot="tbody">
            <tr v-for="(row, i) in rows"
                @mouseenter="hoveringRow=row"
                @mouseleave="hoveringRow=null"
                :class="'row-'+row.no">
                ...
                <template v-if="col.value !== 'actions'">
                  <template v-if="col.editAble!==false">
                    ...
                    <template v-else-if="col.value==='stmt'">
                      <select :class="{lbl: row.stmtType==='LBL'}"
                              name=""
                              v-model="row.stmtType"
                              @focus="focusingRow=row"
                              @blur="focusingRow=null">
                        <option v-for="item in statements"
                                :value="item.value">
                          {{item.text}}
                        </option>
                      </select>
                      <input v-if="row.stmtType==='LBL'"
                             type="number"
                             v-model="row.lbl"
                             class="lbl"
                             @focus="focusingRow=row"
                    ...
                    <input v-else
                           type="text"
                           v-model="row[col.value]"
                           @focus="exprInputFocus($event,row)"
                           @blur="exprInputBlur"
                           class="expr" />
                  </template>
